...
def load_and_process_BigQuery(
    query, project_id, date_col,
    credentials_path="/Users/sebastianwefers/Desktop/DataEng_Crypto_Sentiment/secrets/dataengineering-secret.json"
    ):
...
def load_data(path1:str, path2:str=None, path3=None, path4=None):
    data1 = pd.read_parquet(path1) if path1 else None
    data2 = pd.read_parquet(path2) if path2 else None
    data3 = pd.read_parquet(path3) if path3 else None
    data4 = pd.read_parquet(path4) if path4 else None
